...
class AttachmentProtocol(BaseProtocol):
    def create(
        self,
        contributions: NonEmptyList[Attachment | dict[str, Any]],
    ...
    def query(
        ...
        sort: str | None = None,
        _timeout: int = -1,
    ) -> list[Attachment] | pagination.Paginator: ...
    def update(
        self,
        data: Attachment | dict[str, Any],
        query: dict[str, Any] | None = None,
        _timeout: int = -1,
    ) -> int: ...
    def upsert(
        self,
        data: NonEmptyList[Attachment | dict[str, Any]],
        allow_duplicates: bool = False,
    ) -> UpsertResponse: ...
    def remove(self, query: dict[str, Any], _timeout: int = -1) -> int: ...


class AttachmentResource(BaseResource, AttachmentProtocol):
    def __init__(
        self,
        http: httpx.Client,
        use_document_model: bool = True,
        endpoint_slug: str = "attachments",
    ) -> None:
